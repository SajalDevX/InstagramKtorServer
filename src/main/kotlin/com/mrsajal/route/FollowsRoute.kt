        ...
        route(path = "/follows") {
            post(path = "/follow") {
                try {
                    ...
                    if (params == null) {
                        call.respond(
                            status = HttpStatusCode.BadRequest, message = FollowAndUnfollowResponse(
                                success = false, message = Constants.MISSING_PARAMETERS_ERROR_MESSAGE
                            )
                        )
                        return@post
                    }
                    ...
                    call.respond(
                        status = HttpStatusCode.InternalServerError, message = FollowAndUnfollowResponse(
                            success = false, message = Constants.UNEXPECTED_ERROR_MESSAGE
                        )
                    )
                }
            }
            ...
            post(path = "/unfollow") {
                try {
                    ...
                    call.respond(
                        status = HttpStatusCode.InternalServerError, message = Constants.UNEXPECTED_ERROR_MESSAGE
                    )
                }
            }
            ...
            get(path = "/followers") {
                try {
                    ...
                    call.respond(
                        status = HttpStatusCode.BadRequest, message = Constants.MISSING_PARAMETERS_ERROR_MESSAGE
                    )
                } catch (anyError: Throwable) {
                    call.respond(
                        status = HttpStatusCode.InternalServerError, message = Constants.UNEXPECTED_ERROR_MESSAGE
